...
function MaterialCardItem({item, studyTypeContent, course, refreshData}) {
  ...
  const GenerateContent = async() => {
    ...
    toast('Your content has generated successfully')
  }

  return (
    <Link href={'/course/'+course?.courseId+item.path}>
      <div className={`border shadow-md rounded-lg p-5 flex flex-col items-center
      ${(studyTypeContent?.[item.type]?.length == null && studyTypeContent?.[item.type]?.content?.length == null) && 'grayscale'}
    `}>
        {(studyTypeContent?.[item.type]?.length == null && studyTypeContent?.[item.type]?.content?.length == null) ? <h2 className='p-1 px-2 bg-gray-500 text-white rounded-full text-[10px] mb-2'>Generating</h2>
        : <h2 className='p-1 px-2 bg-green-500 text-white rounded-full text-[10px] mb-2'>Ready</h2> }
        <Image src={item.icon} alt={item.name} width={50} height={50}/>
        <h2 className='font-medium mt-3'>{item.name}</h2>
        <p className='text-gray-500 text-sm text-center'>{item.desc}</p>

        {(studyTypeContent?.[item.type]?.length == null && studyTypeContent?.[item.type]?.content?.length == null) ? 
